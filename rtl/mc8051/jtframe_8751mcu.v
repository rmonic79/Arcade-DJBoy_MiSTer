...
// djboy_dpram (altsyncram): jtframe_dual_ram_cen non veniva inferito da Quartus 17.0
// ("asynchronous read logic") → 32k registri. clocken port B = cen_eff (gating letture MCU).
djboy_dpram #(.AW(12),.DW(8)) u_prom(
    .clk    ( clk       ),
    // Port A: programmazione ROM (download)
    .cen_a  ( 1'b1      ),
    .addr_a ( prog_addr ),
    .d_a    ( prom_din  ),
    .we_a   ( prom_we   ),
    .q_a    (           ),
    // Port B: fetch MCU (clock-gated su cen)
    .cen_b  ( cen_eff   ),
    .addr_b ( rom_addr[11:0] ),
    .d_b    ( 8'd0      ),
    .we_b   ( 1'b0      ),
    .q_b    ( rom_data  )
);
